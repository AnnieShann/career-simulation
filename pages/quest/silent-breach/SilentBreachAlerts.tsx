...
import { AlertTriangle, ArrowRight, Activity, Shield, CheckCircle2 } from "lucide-react";
...
const team: TeamMember[] = [
  {
    id: "jack", name: "Jack", role: "Cybersecurity Engineer", avatar: avatarJack, color: "#22D3EE",
    intro: [
      "I've been monitoring server logs for the past two hours. Every 10 minutes, there's an outbound data transfer to an unfamiliar IP address.",
      "It's consistent. Automated. That's what worries me.",
    ],
    options: [
  ...
  {
    id: "emma", name: "Emma", role: "Project Manager", avatar: avatarAlyssa, color: "#E879F9",
  ...
  {
    id: "lily", name: "Lily", role: "Threat Analyst", avatar: avatarMia, color: "#34D399",
  ...
  {
    id: "mike", name: "Mike", role: "Incident Response Specialist", avatar: avatarOlivia, color: "#FF4D4D",
...
const SilentBreachAlerts = () => {
  ...
  const startTalk = (member: TeamMember) => {
  ...
  const advanceIntro = () => {
    ...
    if (introIdx < activeMember.intro.length - 1) {
      setIntroIdx(introIdx + 1);
    } else {
      if (activeMember.options.length > 0) setShowOptions(true);
      else finishMember();
    }
  };
  ...
  const advanceResponse = () => {
    if (!activeMember || chosenOption === null) return;
    const lines = activeMember.options[chosenOption].response;
    if (responseIdx < lines.length - 1) setResponseIdx(responseIdx + 1);
    else finishMember();
  };

  const finishMember = () => {
    if (activeMember && !talkedTo.includes(activeMember.id)) {
      setTalkedTo((p) => [...p, activeMember.id]);
    }
    setPhase("roster");
  ...
  const networkLogsPanel = phase === "talking" && activeMember ? (
    <motion.div className={`${holoPanel} rounded-2xl p-3`} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
      <div className="flex items-center gap-1.5 mb-2">
        ...
        <span className="text-[9px] text-white/40 uppercase tracking-widest font-semibold">Network Logs</span>
      </div>
      {[
  ...
  const threatIntelPanel = phase === "talking" && activeMember ? (
    <motion.div className={`${holoPanel} rounded-2xl p-3`} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
      <div className="flex items-center gap-1.5 mb-2">
        ...
        <span className="text-[9px] text-white/40 uppercase tracking-widest font-semibold">Threat Intel</span>
      </div>
  ...
  const headerExtra = (
    <div className="flex items-center gap-1">
      {team.map((m) => (
        <div key={m.id} className={`w-2 h-2 rounded-full transition-colors ${talkedTo.includes(m.id) ? "bg-emerald-400" : "bg-white/15"}`} />
      ))}
    </div>
  );
  ...
  return (
    <SilentBreachRoomLayout
      guideStep="alerts"
      guideCompleted={["party", "briefing"]}
      guideAction={allDone ? "All team members briefed — complete the room" : `Talk to your team (${talkedTo.length}/${team.length} briefed)`}
      guideHint="Click each team member to hear their intel"
      headerIcon={<AlertTriangle className="w-4 h-4 text-[#FF4D4D]" strokeWidth={1.5} />}
      headerTitle="Meet the Team"
      headerSubtitle="Free Exploration"
      headerExtra={headerExtra}
      leftPanel={networkLogsPanel}
      rightPanel={threatIntelPanel}
      bgSrc={bgSrc}
      bgOpacity={60}
    >
      {/* ROSTER PHASE */}
      <AnimatePresence>
        {phase === "roster" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center gap-6 w-full max-w-3xl">
            <div className="grid grid-cols-4 gap-3 w-full">
              {team.map((member) => {
                ...
                return (
                  <motion.button
                    key={member.id}
                    onClick={() => startTalk(member)}
                    className={`${holoPanel} rounded-2xl p-3 flex flex-col items-center gap-2 cursor-pointer transition-all hover:scale-[1.03]`}
                    style={{ borderColor: done ? "#34D39940" : `${member.color}30`, boxShadow: done ? "none" : `0 0 30px ${member.color}15` }}
                    whileHover={{ y: -4 }}
                  ...
                  >
                    ...
                    <div className="text-center">
                      <p className="text-xs font-bold" style={{ color: member.color }}>{member.name}</p>
                      <p className="text-[8px] text-white/40">{member.role}</p>
                    </div>
                    <div className={`rounded-full px-2.5 py-0.5 text-[8px] font-semibold ${done ? "bg-emerald-500/10 text-emerald-400" : "bg-white/5 text-white/50"}`}>
                      {done ? "Briefed ✓" : "Talk"}
                    </div>
                  </motion.button>
                );
            ...
            {/* Continue safe zone: 32px padding around button */}
            {allDone && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="pt-4 pb-8">
                <button onClick={handleComplete} className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#22D3EE] to-[#6C8CFF] text-white text-sm font-semibold flex items-center gap-2 hover:shadow-[0_0_30px_rgba(34,211,238,0.4)] transition-shadow">
                  ...
                  <div className="flex-1 flex flex-col justify-between py-2">
                    <div>
                      <div className="flex items-center gap-3 mb-3">
                        <span className="text-lg font-bold" style={{ color: activeMember.color }}>{activeMember.name}</span>
                        <span className="text-sm text-white/30">{activeMember.role}</span>
                      </div>
                      <p className="text-base leading-relaxed text-white/60">{activeMember.intro[introIdx]}</p>
                    </div>
                    {/* Continue button with 32px safe zone */}
                    <div className="flex justify-end mt-6 pb-2">
                      <button onClick={advanceIntro} className="px-5 py-2 rounded-xl bg-gradient-to-r from-[#22D3EE] to-[#6C8CFF] text-white text-sm font-semibold flex items-center gap-2 hover:shadow-[0_0_20px_rgba(34,211,238,0.3)] transition-shadow">
              ...
              {showOptions && (
                <motion.div key="options" className={`w-full max-w-2xl rounded-2xl p-6 ${holoPanel}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                  <div className="flex items-center gap-2 mb-4">
                    <span className="text-sm font-bold text-white/60">Ask {activeMember.name}:</span>
                  </div>
                  <div className="space-y-3">
                    {activeMember.options.map((opt, i) => (
                      <button key={i} onClick={() => pickOption(i)} className="w-full text-left rounded-xl p-3 border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.05] transition-all">
                ...
                <motion.div key={`resp-${responseIdx}`} className={`w-full max-w-2xl rounded-2xl p-6 ${holoPanel} flex gap-6`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                  ...
                  <div className="flex-1 flex flex-col justify-between py-2">
                    <div>
                      ...
                      <p className="text-base leading-relaxed text-white/60">{activeMember.options[chosenOption].response[responseIdx]}</p>
                    </div>
                    <div className="flex justify-end mt-6 pb-2">
                      <button onClick={advanceResponse} className="px-5 py-2 rounded-xl bg-gradient-to-r from-[#22D3EE] to-[#6C8CFF] text-white text-sm font-semibold flex items-center gap-2 hover:shadow-[0_0_20px_rgba(34,211,238,0.3)] transition-shadow">
                        Continue <ArrowRight className="w-4 h-4" />
                      </button>
                    </div>
                  </div>
