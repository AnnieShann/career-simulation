...
const SilentBreachEmail = () => {
  ...
  const guideAction = () => {
    switch (phase) {
      case "report": return "Step 3: Review the threat report on the side panels";
      case "dialogue": return "Listen to Jack and Lily's artifact analysis";
      case "dp2": return "Choose your next investigation step";
      case "reaction2": return "Review your team's reaction";
  ...
  const leftPanel = (
    <motion.div className={`${holoPanel} rounded-2xl p-3`} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
      <div className="flex items-center gap-1.5 mb-2">
        <FileText className="w-3 h-3 text-[#6C8CFF]" />
        <span className="text-[9px] text-white/40 uppercase tracking-widest font-semibold">Threat Report</span>
      </div>
      <div className="rounded-xl p-3 bg-white/[0.02] border border-white/[0.06] space-y-2 text-[9px] text-white/40">
        ...
        <p className="leading-relaxed text-white/50">
          AegisFox used encrypted outbound packets every 20 minutes. Target IP cluster: 185.4.72.xxx. Exfiltration preceded ransomware deployment.
        </p>
  ...
  const rightPanel = (
    <motion.div className={`${holoPanel} rounded-2xl p-3`} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
      <div className="flex items-center gap-1.5 mb-2">
        ...
        <span className="text-[9px] text-white/40 uppercase tracking-widest font-semibold">Packet Timing</span>
      </div>
      <div className="space-y-1.5">
        {[
          { time: "14:00", size: 10, pct: 33 },
          { time: "14:10", size: 15, pct: 50 },
          { time: "14:20", size: 22, pct: 73 },
          { time: "14:30", size: 27, pct: 90 },
          { time: "14:40", size: 30, pct: 100 },
        ].map((d, i) => (
          <motion.div key={i} className="flex items-center gap-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 + i * 0.1 }}>
            ...
            <div className="flex-1 h-2 rounded-full bg-white/[0.06] overflow-hidden">
              <motion.div className="h-full rounded-full bg-gradient-to-r from-amber-400 to-red-400" initial={{ width: 0 }} animate={{ width: `${d.pct}%` }} transition={{ duration: 0.6, delay: 0.4 + i * 0.1 }} />
            </div>
  ...
  return (
    <SilentBreachRoomLayout
      guideStep="email"
      guideCompleted={["party", "briefing", "alerts", "decision"]}
      guideAction={guideAction()}
      guideHint={phase === "report" ? "Review the artifact before choosing how to escalate" : phase === "dp2" ? "Consider how your first decision affects this choice" : undefined}
      headerIcon={<Mail className="w-4 h-4 text-[#6C8CFF]" strokeWidth={1.5} />}
      headerTitle="Mail Analytics"
      headerSubtitle="Evidence & Artifacts"
      leftPanel={leftPanel}
    ...
    >
      ...
      {phase === "report" && (
        <motion.div className={`w-[520px] rounded-2xl p-6 ${holoPanel} text-center`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <button onClick={() => setPhase("dialogue")} className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-[#6C8CFF] to-[#9C6BFF] text-white text-sm font-semibold flex items-center gap-2 mx-auto hover:shadow-[0_0_20px_rgba(108,140,255,0.3)] transition-shadow">
            Analyze the Data <ArrowRight className="w-3 h-3" />
          </button>
        </motion.div>
      )}
          ...
          <motion.div key={dialogStep} className={`max-w-3xl w-full rounded-2xl p-8 ${holoPanel}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
            <div className="flex items-start gap-10">
              ...
              <div className="flex-1 flex flex-col justify-center py-4">
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-xl font-bold" style={{ color: artifactDialogue[dialogStep].speaker.color }}>{artifactDialogue[dialogStep].speaker.name}</span>
                  <span className="text-sm text-white/30">{artifactDialogue[dialogStep].speaker.role}</span>
                </div>
            ...
            <div className="flex items-center justify-between mt-5">
              <div className="flex items-center gap-1">
                {artifactDialogue.map((_, i) => (
                  <div key={i} className={`w-1.5 h-1.5 rounded-full transition-colors ${i <= dialogStep ? "bg-[#6C8CFF]" : "bg-white/15"}`} />
                ))}
              </div>
              <button onClick={() => dialogStep < artifactDialogue.length - 1 ? setDialogStep(dialogStep + 1) : setPhase("dp2")} className="px-5 py-2 rounded-xl bg-gradient-to-r from-[#6C8CFF] to-[#34D399] text-white text-sm font-semibold flex items-center gap-2 hover:shadow-[0_0_20px_rgba(108,140,255,0.3)] transition-shadow">
                {dialogStep < artifactDialogue.length - 1 ? "Continue" : "Decision Point 2"} <ArrowRight className="w-4 h-4" />
              </button>
            </div>
          </motion.div>
        </AnimatePresence>
      ...
      {phase === "dp2" && (
        <motion.div className={`w-[680px] rounded-2xl p-6 ${holoPanel}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-2 mb-4">
            <AlertTriangle className="w-4 h-4 text-[#E879F9]" />
            <span className="text-sm font-bold text-white/80">Decision Point 2: Next Steps</span>
          </div>
            ...
            {dp2Options.map((opt) => (
              <button key={opt.id} onClick={() => { setSelected2(opt.id); setDecision2(opt.label); }}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-[9px] px-2 py-0.5 rounded-full ${opt.risk === "High" ? "bg-red-500/20 text-red-400" : opt.risk === "Medium" ? "bg-amber-500/20 text-amber-400" : "bg-emerald-500/20 text-emerald-400"}`}>
                    Risk: {opt.risk}
                  </span>
                </div>
          ...
          {selected2 && (
            <div className="flex justify-end mt-5">
              <button onClick={() => setPhase("reaction2")} className="px-5 py-2 rounded-xl bg-gradient-to-r from-[#E879F9] to-[#6C8CFF] text-white text-sm font-semibold flex items-center gap-2 hover:shadow-[0_0_20px_rgba(232,121,249,0.3)] transition-shadow">
      ...
      {phase === "reaction2" && chosenOpt && (
        <motion.div className={`w-[640px] rounded-2xl p-6 ${holoPanel}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="flex justify-end mt-5">
            <button onClick={() => setPhase("transition")} className="px-5 py-2 rounded-xl bg-gradient-to-r from-[#E879F9] to-[#6C8CFF] text-white text-sm font-semibold flex items-center gap-2 hover:shadow-[0_0_20px_rgba(232,121,249,0.3)] transition-shadow">
      ...
      {phase === "transition" && (
        <motion.div className={`max-w-3xl w-full rounded-2xl p-8 ${holoPanel}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="flex justify-end mt-5">
            <button onClick={() => { completeRoom("email"); navigate("/quest/silent-breach/team-vote"); }} className="px-5 py-2 rounded-xl bg-gradient-to-r from-[#6C8CFF] to-[#34D399] text-white text-sm font-semibold flex items-center gap-2 hover:shadow-[0_0_20px_rgba(108,140,255,0.3)] transition-shadow">
              Team Vote <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </motion.div>
      )}
