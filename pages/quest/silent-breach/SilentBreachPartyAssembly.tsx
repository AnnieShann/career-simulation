...
const partyRoles = [
  {
    ...
    avatar: avatarJordan,
    task: "Cross-reference the suspicious IP address against known threat databases and identify attacker signatures.",
  },
  {
    color: "#FBBF24",
    emoji: "🟡",
    ...
    avatar: avatarPriya,
    task: "Analyze server log data, identify anomalies in the transfer pattern, and flag the highest-risk timestamp.",
  },
  {
    color: "#A78BFA",
    emoji: "🟣",
    ...
    avatar: avatarSam,
    task: "Audit existing incident documentation, identify gaps in the evidence chain, and assess legal exposure.",
  },
];

type Phase = "intro" | "roles" | "ready";
            ...
            <div className={`max-w-4xl w-full rounded-2xl p-8 ${holoPanel} flex gap-8`}>
              ...
              <div className="flex-1 flex flex-col justify-between py-4">
                ...
                <button
                  ...
                  className="mt-6 w-full py-3.5 rounded-xl bg-gradient-to-r from-[#6C8CFF] to-[#22D3EE] text-white text-sm font-semibold flex items-center justify-center gap-2 hover:shadow-[0_0_20px_rgba(108,140,255,0.3)] transition-shadow"
                >
        ...
        {phase === "roles" && (
          ...
          >
            <div className="max-w-5xl w-full">
              {/* Player role card - highlighted */}
              <motion.div
                className={`${holoPanel} rounded-2xl p-5 mb-4 border-[#6C8CFF]/30`}
                style={{ boxShadow: "0 0 40px rgba(108,140,255,0.15)" }}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              >
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                {partyRoles.map((role, i) => (
                  <motion.div
                    key={role.name}
                    className={`${holoPanel} rounded-2xl p-4`}
              ...
              {/* Alex closing line + CTA */}
              <motion.div
                className={`${holoPanel} rounded-2xl p-6 flex gap-6 items-center`}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-sm text-white/60 leading-relaxed">
                    You'll each investigate independently. When the time comes, you'll share what you found and decide together how to respond.
                  </p>
                </div>
                <button
                  onClick={handleBeginMission}
                  className="px-6 py-3 rounded-xl bg-gradient-to-r from-[#6C8CFF] to-[#22D3EE] text-white text-sm font-semibold flex items-center gap-2 hover:shadow-[0_0_20px_rgba(108,140,255,0.3)] transition-shadow shrink-0"
                >
