...
const SilentBreachOutcome = () => {
  ...
  return (
    <div className="min-h-screen mesh-bg text-foreground overflow-y-auto">
      <SilentBreachGuide
        currentStep="results"
        completedSteps={["party", "briefing", "alerts", "decision", "email", "teamvote", "assessment"]}
        nextAction="Continue to Real Opportunities"
      />
      <div className="relative z-10 max-w-5xl mx-auto px-8 py-10">
        {/* Success Banner */}
        <motion.div className="text-center mb-10" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
          <motion.div
            className="w-20 h-20 rounded-3xl bg-gradient-to-br from-[#6C8CFF] to-[#9C6BFF] flex items-center justify-center mx-auto mb-5 shadow-[0_0_50px_rgba(108,140,255,0.4)]"
          ...
          <p className="text-sm text-muted-foreground">
            Leadership Style: <span className="font-bold text-primary capitalize">{leadershipStyle || "—"}</span>
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-12 gap-6">
          <div className="col-span-7">
            ...
            <div className="bento-card p-6">
              ...
              <motion.div className="flex items-center gap-3 p-3 rounded-xl bg-white/40 border border-white/50"
                initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }}>
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#6C8CFF] to-[#9C6BFF] flex items-center justify-center shrink-0">
                  <Shield className="w-5 h-5 text-white" strokeWidth={1.5} />
                </div>
                <div>
                  <p className="text-sm font-semibold text-foreground/80">Incident Response Leader</p>
                  <p className="text-[11px] text-muted-foreground">Led team through Operation Silent Breach</p>
                </div>
              </motion.div>
            </div>
          </div>
          ...
          <div className="col-span-5 flex flex-col gap-6">
            ...
            <div className="bento-card-gradient p-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-xs text-muted-foreground/60 uppercase tracking-wider font-medium">Leadership Style</p>
              </div>
        ...
        <motion.div className="mt-8 flex justify-center gap-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }}>
          <button onClick={() => navigate("/quest/silent-breach/opportunities")} className="px-10 py-4 btn-premium font-semibold text-sm inline-flex items-center gap-2">
            Unlock Real Opportunities <ArrowRight className="w-4 h-4" />
          </button>
          <button onClick={() => navigate("/opportunity-map")} className="px-8 py-4 rounded-xl border border-primary/20 text-primary font-semibold text-sm inline-flex items-center gap-2 hover:bg-primary/5 transition-colors">
