...
const SilentBreachGuide = ({ currentStep, completedSteps, hint, nextAction }: Props) => {
  ...
  // Only one tooltip at a time: hint takes priority, then action
  const showHint = !!(hint && !hintDismissed);
  const showAction = !!(nextAction && !showHint && !actionDismissed);

  return (
    <>
      {/* Step Progress Bar — top-right, below top bar safe zone (top-12) */}
      <motion.div
      ...
      {/* Single tooltip slot — bottom-left, above bottom dock (bottom-16 safe zone) */}
